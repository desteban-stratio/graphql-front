...
import { Query, Course, QueryTwo } from './types';
...
export class CourseService {
  ...
  getCoursesAndUsers(searchTerm: String) {
    return this.apollo.watchQuery<QueryTwo>({
      pollInterval: 500,
      query: gql`
        query usersAndCourses {
          users: allUsers(searchTerm: ""){
            id
            name
            age
          }
          courses: allCourses(searchTerm: ""){
            id
            title
            author
            description
